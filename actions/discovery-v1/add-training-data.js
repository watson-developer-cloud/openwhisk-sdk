 ...
 * @param {string[]} params.collection_ids - A comma-separated list of collection IDs to
 * be queried against.
 ...
 * @param {string[]} [params.return_fields] - A comma separated list of the portion of
 * the document hierarchy to return.
 * @param {number} [params.offset] - The number of query results to skip at the beginning.
 * For example, if the total number of results that are returned is 10, and the offset is 8,
 * it returns the last two results.
 ...
 * which contains the fields that match the query with `<em></em>` tags around the matching query
 * terms. Defaults to false.
 * @param {string} [params.deduplicate_field] - When specified, duplicate results based on the
 * field specified are removed from the returned results. Duplicate comparison is limited to the
 * current query only, `offset` is not considered. This parameter is currently Beta functionality.
...
function main(params) {
  return new Promise((resolve, reject) => {
    ...
    service.queryNotices_0(params, (err, response) => {
      if (err) {
        reject(err.message);
      } else {
        resolve(response);
