...
describe('[action] ToneAnalyzer', () => {
  ...
  it('should fail if params is undefined', () => {
    ...
    return action.main(params).then(() => {
      assert.fail('Undefined parameters error was not found');
    }).catch((err) => {
      assert(err.message === 'Cannot read property \'username\' of undefined');
    });
  });
  ...
  it('should fail if params is null', () => {
    ...
    return action.main(params).then(() => {
      ...
      assert(err.message === 'Cannot read property \'username\' of null');
    });
  });
  ...
  it('should fail if username is not provided', () => {
    ...
    return action.main(params).then(() => {
      ...
      assert(err.message === 'Argument error: username and password are required unless use_unauthenticated is set');
    });
  });
  ...
  it('should fail if version_date is not provided', () => {
    ...
    return action.main(params).then(() => {
      ...
      assert(err.message === 'Argument error: version_date was not specified, use 2016-05-19');
    });
  });
